...
function ProductPage(props) {
  ...
  const HandlClick = (type) => {
    if (type === "dec")
      setProductQuentity((prev) => (ProductQuentity > 1 ? prev - 1 : prev));
    if (type === "inc")
      setProductQuentity((prev) =>
        ProductQuentity < product.quantity ? prev + 1 : prev
      );
  };
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      dispatch(
        setError(
          error?.response?.data?.message || "error accured while creating order"
        )
      );
    }
